...
func (b repositoryName) UpdateTotalPrice(db *sql.DB, id int, userId int) error {
	...
	row := db.QueryRow("SELECT a.order_form_id, b.view_totalPrice FROM sell_contract a LEFT JOIN (SELECT sell_contract_id, SUM(unitPrice * amount) AS view_totalPrice FROM sell_subitem GROUP BY sell_contract_id) b ON a.id = b.sell_contract_id LEFT JOIN sell_subitem c ON c.sell_contract_id = a.id  WHERE c.id=?", id)
